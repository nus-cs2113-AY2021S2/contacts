            ...
            switch (commandType) {
                case "add":
                    ...
                    if (splitArgs.length == 3 // 3 arguments
                        ...
                        person1[2] = result1;
                        decodeResult = !person1[0].isEmpty() && !person1[1].isEmpty() && !person1[2].isEmpty()
                                && person1[2].contains("@") ? person1 : null;
                    }
                    if (decodeResult == null) {
                        feedback = String.format(
                                "Invalid command format: %1$s " + (System.lineSeparator() + "|| ") + "%2$s", "add",
                    ...
                    count++;
                    feedback = String.format("New person added: %1$s, Phone: %2$s, Email: %3$s", decodeResult[0],
                            decodeResult[1], decodeResult[2]);
                    break;
                case "list":
                    ...
                    for (int i = 0; i < count; i++) {
                        ...
                        messageAccumulator.append('\t').append(String.format("%1$d. ", displayIndex)).append(
                                String.format("%1$s  Phone Number: %2$s  Email: %3$s", person[0], person[1], person[2]))
                ...
                case "help":
                    feedback = (String.format("%1$s: %2$s", "add", "Adds a person to contacts.")
                            ...
                            + (String.format("%1$s: %2$s", "exit", "Exits the program.")
                                    + (System.lineSeparator() + "|| ") + String.format("\tExample: %1$s", "exit"))
                            + (System.lineSeparator() + "|| ") + (System.lineSeparator() + "|| ")
                            + (String.format("%1$s: %2$s", "help", "Shows program usage instructions.")
                                    + (System.lineSeparator() + "|| ") + String.format("\tExample: %1$s", "help"));
                    break;
                case "exit":
                ...
                default:
                    feedback = String.format(
                            "Invalid command format: %1$s " + (System.lineSeparator() + "|| ") + "%2$s", commandType,
                            (String.format("%1$s: %2$s", "add", "Adds a person to contacts.")
                                    + (System.lineSeparator() + "|| ")
                                    + String.format("\tParameters: %1$s",
                                            "NAME " + "p/" + "PHONE_NUMBER " + "e/" + "EMAIL")
